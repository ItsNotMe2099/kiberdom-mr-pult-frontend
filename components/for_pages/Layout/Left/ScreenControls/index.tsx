...
export default function ScreenControls({color, options, indexScreen}: Props) {
  ...
  const handleClickOption = async (index: number) => {
    ...
    if (index === 0) {
      await LedRepository.setContentState(LedState.Gallery, +indexScreen)
    }
    if (index === 1) {
      await LedRepository.setContentState(LedState.Content, +indexScreen)
    }
  }
      ...
      <div className={styles.bottom} style={isActive ? {background: getColor(color, false, false, true)} : {}}>
        {options?.map((i, indexOption) =>
          <div
            ...
            style={isActive && i.value === option ? {background: getColor(color, false, false, false, true)} : {}}
            key={i.label}>
            {i.img}
